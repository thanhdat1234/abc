...
<div class="row">
    <div class="col-sm-12">
    	<form action="<?php echo base_url('admin/product/bulk_action');?>" method="post" id="frm-bulk-action">
                ...
                <table  class="table table-bordered table-hover" >
                    <thead>
                        <tr>
                            ...
                            <th style="width: 120px; background: #3c8dbc; color: white;"><?php front_lang('product_name');?></th>                         
                            <th style="width: 150px; background: #3c8dbc; color: white;"><?php front_lang('description');?></th>
                    ...
                    <tbody id="result">
                            ...
                            <?php foreach($result as $value): ?>
                                <tr id="record-<?php echo $value['id'];?>">
                                    ...
                                    <td ><img src="<?php echo base_url('upload/product').'/'.$value['avarta'];?>" width="100px" height="150px" alt=""></td>
                                    <td >
                                        <p><?php echo $value['product_name'];?></p>
                                        <p>Giá: <?php echo number_format($value['price'],0,'','.');?> VNĐ</p>
                                        <p>Lượt xem: <?php echo $value['view'];?></p>
                                        <p>Lượt mua: <?php echo $value['checkout'];?></p>
                                        <p>Yêu thích: <?php echo $value['like'];?></p>
                                    </td>
                                    ...
                                    <td>
                                        ...
                                        <button class="btn btn-info btn-xs" onclick="Product_Edit('<?php echo (int)$value["id"] ?>');" type="button">
                                            <i class="fa fa-pencil"></i>
                                        </button>
                                        <button class="btn btn-danger btn-xs" onclick="Product_Delete('<?php echo (int)$value["id"] ?>');" type="button">
...
<script>
    ...
    function Add(){
        $(".content").mask("<?php echo 'Loading...'; ?>");
        $('.content').load("<?php echo base_url('admin/product/add');?>",function(){
            $(".contentt").unmask();
        });
    }
    ...
    function Product_Edit(id){
        ...
        var url = '<?php echo base_url("admin/product/edit");?>'+'/'+id+'/'+curent_page;
    ...
    function change_status(id,status){
        $(".content").mask("<?php echo 'Loading...'; ?>");
        var base_url = '<?php echo base_url("admin/product/change_status");?>'; 
    ...
    function Product_Delete(id){
     $.sModal({
            ...
            content:'<?php echo "Do you want to delete this product ?"; ?>',
            animate:'fadeDown',
            buttons:[
                        {
                            ...
                            click:function(m_id,data){
                                $(".content").mask("<?php echo 'Loading...'; ?>");
                                var url = '<?php echo base_url("admin/product/delete");?>'+'/'+id;
                                $.get(url,function(result) {
                                    if(result.error == 0){
