...
<div class="row">
    <div class="col-sm-12">
    	<form action="<?php echo base_url('admin/banner/bulk_action');?>" method="post" id="frm-bulk-action">
                ...
                <table  class="table table-bordered table-hover" >
                    <thead>
                        <tr>
                            ...
                            <th style="width: 100px; background: #3c8dbc; color: white;"><?php front_lang('link');?></th>
                            <?php if($type == 3){?>
                            <th style="width: 100px; background: #3c8dbc; color: white;"><?php echo "Cột";?></th>      
                            <?php }?>                         
                    ...
                    <tbody id="result">
                            ...
                            <?php foreach($result as $value): ?>
                                <tr id="record-<?php echo $value['id'];?>">
                                    ...
                                    <td >
                                        <?php if($type == 3){ ?>
                                            <img src="<?php echo base_url('upload/banner').'/'.$value['images'];?>"  alt="">
                                        <?php }else { ?>
                                            <img src="<?php echo base_url('upload/banner').'/'.$value['images'];?>" width="100px" height="150px" alt="">
                                        <?php }?>
                                    </td>
                                    <td >
                                    ...
                                    <td>
                                        ...
                                        <button class="btn btn-info btn-xs" onclick="banner_Edit('<?php echo (int)$value["id"] ?>');" type="button">
                                            <i class="fa fa-pencil"></i>
                                        </button>
                                        <button class="btn btn-danger btn-xs" onclick="banner_Delete('<?php echo (int)$value["id"] ?>');" type="button">
...
<script>
    ...
    function Add(){
        $(".content").mask("<?php echo 'Loading...'; ?>");
        $('.content').load("<?php echo base_url('admin/banner/add').'/'.$type;?>",function(){
            $(".contentt").unmask();
        });
    }
    ...
    function banner_Edit(id){
        ...
        var url = '<?php echo base_url("admin/banner/edit");?>'+'/'+id+'/'+curent_page;
    ...
    function change_status(id,status){
        $(".content").mask("<?php echo 'Loading...'; ?>");
        var base_url = '<?php echo base_url("admin/banner/change_status");?>'; 
    ...
    function banner_Delete(id){
     $.sModal({
            ...
            content:'<?php echo "Do you want to delete this banner ?"; ?>',
            animate:'fadeDown',
            buttons:[
                        {
                            ...
                            click:function(m_id,data){
                                $(".content").mask("<?php echo 'Loading...'; ?>");
                                var url = '<?php echo base_url("admin/banner/delete")."/".$type;?>'+'/'+id;
                                $.get(url,function(result) {
                                    if(result.error == 0){
