...
<div class="row">
    <div class="col-sm-12">
    	<form action="<?php echo base_url('admin/news/bulk_action');?>" method="post" id="frm-bulk-action">
                ...
                <table  class="table table-bordered table-hover" >
                    <thead>
                        <tr>
                            ...
                            <th style="width: 20px; background: #3c8dbc; color: white;"><?php front_lang('avarta');?></th>
                            <th style="width: 120px; background: #3c8dbc; color: white;"><?php front_lang('title');?></th>                         
                            <th style="width: 200px; background: #3c8dbc; color: white;"><?php front_lang('description');?></th>
                            <th style="width: 100px; background: #3c8dbc; color: white;"><?php front_lang('category');?></th>
                    ...
                    <tbody id="result">
                            ...
                            <?php foreach($result as $value): ?>
                                <tr id="record-<?php echo $value['id'];?>">
                                    ...
                                    <td >
                                        <?php if($value['images'] != NULL){?>
                                            <img src="<?php echo base_url().$value['images'];?>" width="100px" height="150px" alt="">
                                        <?php } else { ?>
                                           <img src="<?php echo base_url().$value['avarta'];?>" width="100px" height="150px" alt="">
                                        <?php } ?>
                                    </td>
                                    <td >
                                    ...
                                    <td>
                                        <button class="btn btn-info btn-xs" onclick="news_Edit('<?php echo (int)$value["id"] ?>');" type="button">
                                            <i class="fa fa-pencil"></i>
                                        </button>
                                        <button class="btn btn-danger btn-xs" onclick="news_Delete('<?php echo (int)$value["id"] ?>');" type="button">
...
<script>
    ...
    function Add(){
        $(".content").mask("<?php echo 'Loading...'; ?>");
        $('.content').load("<?php echo base_url('admin/news/add');?>",function(){
            $(".contentt").unmask();
        });
    }
    ...
    function news_Edit(id){
        ...
        var url = '<?php echo base_url("admin/news/edit");?>'+'/'+id+'/'+curent_page;
    ...
    function change_status(id,status){
        $(".content").mask("<?php echo 'Loading...'; ?>");
        var base_url = '<?php echo base_url("admin/news/change_status");?>'; 
            ...
            buttons:[
                        {
                            ...
                            click:function(m_id,data){
                                $(".content").mask("<?php echo 'Loading...'; ?>");
                                var url = '<?php echo base_url("admin/news/delete");?>'+'/'+id;
                                $.get(url,function(result) {
                                    if(result.error == 0){
